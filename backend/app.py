...
load_dotenv()   # reads .env automatically — must be before app config
...
SECRET_KEY   = os.environ.get('SECRET_KEY',   'bookswap_fallback_secret')
DATABASE_URL = os.environ.get('DATABASE_URL',  'postgresql://postgres:1234@localhost:5432/bookswap_db')
...
CORS(
    ...
    origins=[
        "http://localhost:5173",
        "https://bookswap-project.vercel.app",
        "https://*.vercel.app",
        FRONTEND_URL,
    ]
)
...
google = oauth.register(
    name='google',
    client_id=os.environ.get('GOOGLE_CLIENT_ID'),           # ← from .env
    client_secret=os.environ.get('GOOGLE_CLIENT_SECRET'),   # ← from .env
...
def google_login():
    redirect_uri = BACKEND_URL + '/api/google_auth'         # ← from .env
...
def google_auth():
    try:
        ...
        return redirect(FRONTEND_URL + '/login?error=google_failed')   # ← from .env
    ...
    if not email:
        return redirect(FRONTEND_URL + '/login?error=no_email')   # ← from .env
    ...
    return redirect(FRONTEND_URL + '/')   # ← from .env
...
def forgot_password():
    ...
    if user:
        ...
        reset_link = f"{FRONTEND_URL}/reset_password?token={token}&email={email}"   # ← from .env
        ...
        db.session.add(ev)
        db.session.commit()
    return jsonify({"message": "If this email is registered, you will receive reset instructions."})
...
def books():
    ...
    nearby    = request.args.get('nearby', 'false')
    min_price = request.args.get('min_price', type=float)
    max_price = request.args.get('max_price', type=float)

    query = Book.query.filter_by(available=True)   # Show all available books
...
def book_detail(book_id):
    try:
        ...
        avg_rating = round(
            sum(r.rating for r in seller_reviews) / len(seller_reviews), 1
        ) if seller_reviews else None
...
def add_book():
    ...
    book = Book(
        ...
        is_approved=True    # auto-approved, no admin review needed
    )

    db.session.add(book)
    db.session.commit()
    return jsonify({"message": "Book listed successfully! It is now live."})
...
@login_required
def orders():
    user_orders = Order.query.filter_by(buyer_id=current_user.id)\
    ...
    for o in user_orders:
        ...
        result.append({
            ...
            "tracking_note":   o.tracking_note   or "",
...
def cancel_order(order_id):
    ...
    send_notification(
        current_user.id,
        f"Order for '{book.title if book else 'book'}' cancelled. ₹{order.amount} refunded.",
        "info"
    )
...
def raise_dispute():
    ...
    for admin in admins:
        send_notification(
            admin.id,
            f"New dispute raised by {current_user.username} for order #{order_id}.",
            "dispute"
        )
...
def send_verification_email():
    ...
    ev = EmailVerification(user_id=current_user.id, token=token, expires_at=expires)
    db.session.add(ev)
    db.session.commit()

    verify_link = f"{BACKEND_URL}/api/verify_email/{token}"   # ← from .env
...
def send_phone_otp():
    ...
    phone_otp = PhoneOTP(user_id=current_user.id, otp=otp, expires_at=expires)
...
def chat(user_id):
    ...
    Message.query.filter_by(
        receiver_id=current_user.id, sender_id=user_id, is_read=False
    ).update({"is_read": True})
    db.session.commit()
...
def get_tracking(order_id):
    order = Order.query.get_or_404(order_id)
    if (order.buyer_id  != current_user.id and
...
def my_referral_code():
    ...
    return jsonify({
        ...
        "referral_link": f"{FRONTEND_URL}/register?ref={current_user.referral_code}"   # ← from .env
    })
...
def apply_referral():
    ...
    db.session.add(WalletTransaction(
        user_id=referrer.id, amount=bonus, txn_type="credit",
        description=f"Referral bonus — {current_user.username} joined",
        balance_after=referrer.wallet_balance
    ))
    db.session.add(WalletTransaction(
        user_id=current_user.id, amount=bonus, txn_type="credit",
...
def seller_store(seller_id):
    ...
    books   = Book.query.filter_by(seller_id=seller_id, is_approved=True)\
                  .order_by(Book.created_at.desc()).all()
    reviews = Review.query.filter_by(reviewed_user_id=seller_id)\
                  .order_by(Review.created_at.desc()).all()
    ...
    return jsonify({
        ...
        "reviews": [{
            ...
            "reviewer": User.query.get(r.reviewer_id).username
                        if User.query.get(r.reviewer_id) else "User",
            "created_at": r.created_at.isoformat()
        } for r in reviews[:5]]
    })
...
if __name__ == "__main__":
    ...
    debug = FLASK_ENV == "development"   # ← from .env: development=True, production=False
